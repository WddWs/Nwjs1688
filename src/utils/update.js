'use strict'
import { exeCode } from './exeCode.js'
...
var copyPath, execPath, upd, loadingInstance;
...
export function checkUpdate() {
    loadingInstance = Loading.service({ fullscreen: true, text: '检查更新中' });
    getUpdateJson().then(json => {
        ...
        if (gui.App.argv.length) {
            ...
            upd.checkNewVersion(function(error, newVersionExists, manifest) {
                ...
                alert("发现新版本！！")
                loadingInstance.close();
                loadingInstance = Loading.service({ text: '下载中' });
                upd.download(function(error, filename) {
                    if (!error) {
                        //解压临时目录的最新压缩包
                        loadingInstance.close();
                        loadingInstance = Loading.service({ text: '解压中' });
                ...
                stat(_src, function(err, st) {
                    ...
                    if (st.isFile()) {
                        ...
                        readable.pipe(writable).on('close', () => {
                            ...
                            // loadingInstance.close();
                            // loadingInstance = Loading.service({ text: '安装中' + ((errorCount / 19) * 100).toFixed(0) + '%' });
